...
- **Review by email (agreed, not built).** When candidates land (mined or proposed on the fly), email the owner a short digest — kind, title, one evidence line — with accept/reject links; email is a convenience layer over the same queue, never a second source of truth. Transport stays stdlib (`smtplib` / `urllib`, no mail library). The setup must not assume anything the user's machine already has:
  - `poppy email setup --provider <name> --from <addr> [--user <addr>]` fills host/port/TLS from a small built-in preset table (the big consumer providers, a `generic` SMTP escape hatch, and the one-key HTTP senders), takes the credential with hidden input, writes Poppy's 0600 config, and `poppy email test` sends a probe and reports the provider's actual error. `--json` throughout so the installing agent can drive and verify it.
  - The installer prompt asks whether the user wants review email and walks them through the one un-automatable step (creating an app password / API key), the same way it already handles models and transcript stores. Providers with one-key HTTP APIs (e.g. Resend, Postmark, Mailgun) are the candidate default when SMTP AUTH proves to be the annoying part.
  - Conventional `SMTP_*` environment variables remain supported as an override for servers and existing setups, but they are a convenience, not the documented path. A zero-account push channel (e.g. ntfy) is the fallback for users who want notifications without a mailbox.
  - Links carry signed, single-use tokens bound to the UI token; a bare GET never mutates state — the link opens a small confirmation page that POSTs the decision and shows the result.
